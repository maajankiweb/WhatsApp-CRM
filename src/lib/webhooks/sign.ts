...
//
// Every delivery carries an `X-Wachatra-Signature` header so receivers
// can verify the request really came from Wachatra and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-Wachatra-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-Wachatra-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so a Wachatra-to-Wachatra integration
