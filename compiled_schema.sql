...
--   - Self-hosted Wachatra instances are multi-user (small teams, shared
...
--
-- Wachatra is single-tenant per WhatsApp number by design (RLS on
...
--   Wachatra user adds a second number under the same WABA, the UI
...
--
-- Turns Wachatra from single-tenant-per-user into multi-tenant-per-
...
-- assumed USD. Wachatra is self-hostable and used globally, so a fixed
...
-- dashboard, idle, or gone. Implements Wachatra#269.
--
-- Design
--
...
--     (`wachatra_live_a1b2c3d4`) so the dashboard can show "which key
...
CREATE TABLE IF NOT EXISTS api_keys (
  ...
  name         text NOT NULL,
  key_prefix   text NOT NULL,             -- display only, e.g. "wachatra_live_a1b2c3d4"
...
--
-- Lets an account register HTTPS endpoints that Wachatra POSTs to when
...
CREATE TABLE IF NOT EXISTS ai_knowledge_chunks (
  ...
  content      text NOT NULL,
  -- Language-neutral FTS config: Wachatra is used in many languages
